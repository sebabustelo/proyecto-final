...
class ProductosArchivosController extends AppController
{
    ...
    {
        $productosArchivo = $this->ProductosArchivos->get($id, contain: []);
        if ($this->request->is(['patch', 'post', 'put'])) {
            $productosArchivo = $this->ProductosArchivos->patchEntity($productosArchivo, $this->request->getData());
            if ($this->ProductosArchivos->save($productosArchivo)) {
                $this->Flash->success(__('The productos archivo has been saved.'));

                return $this->redirect(['action' => 'index']);
            }
            $this->Flash->error(__('The productos archivo could not be saved. Please, try again.'));
        }
        $productos = $this->ProductosArchivos->Productos->find('list', limit: 200)->all();
        $this->set(compact('productosArchivo', 'productos'));
    }
